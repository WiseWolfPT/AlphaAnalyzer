...
export class ValuationService {
  ...
  /**
   * FASE 3.1: Calculate P/B Median 5Y
   * Formula: IV = Median_PB_5y × Book_Value_per_Share_TTM
   */
  async calculatePBMedian5Y(ticker: string): Promise<PBValuationResponse | null> {
    const upperTicker = ticker.toUpperCase();
    const cacheKey = VALUATION_CACHE_KEYS.IV_CALC + upperTicker + ':pb_median';

    // Check cache
    const cached = await redisCacheService.get<PBValuationResponse>(cacheKey);
    if (cached) {
      logger.info(`[ValuationService] P/B Median cache hit for ${upperTicker}`);
      return cached;
    }
    ...
    try {
      ...
      const medianIdx = Math.floor(pbRatios.length / 2);
      const medianPB = pbRatios.length % 2 === 0
        ? (pbRatios[medianIdx - 1] + pbRatios[medianIdx]) / 2
        : pbRatios[medianIdx];
      ...
      // Calculate intrinsic value
      const iv = medianPB * bookValuePerShare;

      if (!isFinite(iv) || iv <= 0) {
        return null;
      }

      logger.info(`[ValuationService] P/B Median for ${upperTicker}: Median=${medianPB.toFixed(2)}, BVPS=${bookValuePerShare.toFixed(2)}, IV=$${iv.toFixed(2)}`);
      ...
      logger.error(`[ValuationService] Error calculating P/B Median for ${upperTicker}:`, error.message);
      return null;
    }
  }
  ...
  /**
   * FASE 3.1: Calculate P/B Median without NRI (Non-Recurring Items)
   * Formula: IV = Median_PB_without_NRI_5y × Adjusted_BVPS_TTM
   */
  async calculatePBMedianWithoutNRI(ticker: string): Promise<PBValuationResponse | null> {
    const upperTicker = ticker.toUpperCase();
    const cacheKey = VALUATION_CACHE_KEYS.IV_CALC + upperTicker + ':pb_median_no_nri';

    // Check cache
    const cached = await redisCacheService.get<PBValuationResponse>(cacheKey);
    if (cached) {
      logger.info(`[ValuationService] P/B Median without NRI cache hit for ${upperTicker}`);
      return cached;
    }
    ...
    try {
      ...
      // Calculate median P/B
      const sortedPB = adjustedPBRatios.sort((a, b) => a - b);
      const medianIdx = Math.floor(sortedPB.length / 2);
      const medianPB = sortedPB.length % 2 === 0
        ? (sortedPB[medianIdx - 1] + sortedPB[medianIdx]) / 2
        : sortedPB[medianIdx];
      ...
      logger.info(`[ValuationService] P/B Median without NRI for ${upperTicker}: Median=${medianPB.toFixed(2)}, BVPS=${bookValuePerShare.toFixed(2)}, IV=$${iv.toFixed(2)}`);
      ...
      logger.error(`[ValuationService] Error calculating P/B Median without NRI for ${upperTicker}:`, error.message);
      return null;
    }
  }
  ...
  /**
   * FASE 3.1: Calculate P/E Median 5Y
   * Formula: IV = Median_PE_5y × EPS_TTM
   */
  async calculatePEMedian5Y(ticker: string): Promise<PEValuationResponse | null> {
    const upperTicker = ticker.toUpperCase();
    const cacheKey = VALUATION_CACHE_KEYS.IV_CALC + upperTicker + ':pe_median';

    // Check cache
    const cached = await redisCacheService.get<PEValuationResponse>(cacheKey);
    if (cached) {
      logger.info(`[ValuationService] P/E Median cache hit for ${upperTicker}`);
      return cached;
    }
    ...
    try {
      ...
      const medianIdx = Math.floor(peRatios.length / 2);
      const medianPE = peRatios.length % 2 === 0
        ? (peRatios[medianIdx - 1] + peRatios[medianIdx]) / 2
        : peRatios[medianIdx];
      ...
      logger.info(`[ValuationService] P/E Median for ${upperTicker}: Median=${medianPE.toFixed(2)}, EPS=${epsTTM.toFixed(2)}, IV=$${iv.toFixed(2)}`);
      ...
      logger.error(`[ValuationService] Error calculating P/E Median for ${upperTicker}:`, error.message);
      return null;
    }
  }

  /**
   * FASE 3.1: Calculate P/E Median without NRI
   * Formula: IV = Median_PE_without_NRI_5y × Adjusted_EPS_TTM
   */
  async calculatePEMedianWithoutNRI(ticker: string): Promise<PEValuationResponse | null> {
    const upperTicker = ticker.toUpperCase();
    const cacheKey = VALUATION_CACHE_KEYS.IV_CALC + upperTicker + ':pe_median_no_nri';

    // Check cache
    const cached = await redisCacheService.get<PEValuationResponse>(cacheKey);
    if (cached) {
      logger.info(`[ValuationService] P/E Median without NRI cache hit for ${upperTicker}`);
      return cached;
    }
    ...
    try {
      ...
      // Fetch historical ratios ONCE (not inside loop) - BUG FIX
      const ratiosData = await fmpGet<any[]>(`/api/v3/ratios/${upperTicker}`, { limit: 5 });
      if (!ratiosData || !Array.isArray(ratiosData)) {
        logger.warn(`[ValuationService] No ratios data for ${upperTicker}`);
        return null;
      ...
      for (const stmt of incomeData) {
        ...
        if (shares > 0 && adjustedNetIncome > 0) {
          // Use already-fetched ratiosData (not fetching inside loop)
      ...
      // Calculate median P/E
      const sortedPE = adjustedPERatios.filter(pe => pe > 0 && pe < 100).sort((a, b) => a - b);
      const medianIdx = Math.floor(sortedPE.length / 2);
      const medianPE = sortedPE.length % 2 === 0
        ? (sortedPE[medianIdx - 1] + sortedPE[medianIdx]) / 2
        : sortedPE[medianIdx];
      ...
      const response: PEValuationResponse = {
        ...
        currentPrice,
        eps: adjustedEPS,
        historicalPE: sortedPE,
        excludeNRI: true,
        confidence: 'MED',
        as_of: new Date().toISOString().split('T')[0],
      };

      logger.info(`[ValuationService] P/E Median without NRI for ${upperTicker}: Median=${medianPE.toFixed(2)}, Adj_EPS=${adjustedEPS.toFixed(2)}, IV=${iv.toFixed(2)}`);
      ...
      logger.error(`[ValuationService] Error calculating P/E Median without NRI for ${upperTicker}:`, error.message);
      return null;
    }
  }

  /**
   * FASE 3.1: Calculate P/S Median 5Y
   * Formula: IV = Median_PS_5y × Sales_per_Share_TTM
   */
  async calculatePSMedian5Y(ticker: string): Promise<PSValuationResponse | null> {
    const upperTicker = ticker.toUpperCase();
    const cacheKey = VALUATION_CACHE_KEYS.IV_CALC + upperTicker + ':ps_median';

    // Check cache
    const cached = await redisCacheService.get<PSValuationResponse>(cacheKey);
    if (cached) {
      logger.info(`[ValuationService] P/S Median cache hit for ${upperTicker}`);
      return cached;
    }
    ...
    try {
      ...
      const medianIdx = Math.floor(psRatios.length / 2);
      const medianPS = psRatios.length % 2 === 0
        ? (psRatios[medianIdx - 1] + psRatios[medianIdx]) / 2
        : psRatios[medianIdx];
      ...
      // Calculate intrinsic value
      const iv = medianPS * salesPerShare;

      if (!isFinite(iv) || iv <= 0) {
        return null;
      }

      logger.info(`[ValuationService] P/S Median for ${upperTicker}: Median=${medianPS.toFixed(2)}, SPS=${salesPerShare.toFixed(2)}, IV=$${iv.toFixed(2)}`);
      ...
      logger.error(`[ValuationService] Error calculating P/S Median for ${upperTicker}:`, error.message);
      return null;
    }
  }
