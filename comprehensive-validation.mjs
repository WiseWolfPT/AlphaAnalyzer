  ...
  async validateRealDataAPIs() {
    ...
    for (const endpoint of marketDataEndpoints) {
      ...
      if (result) {
        ...
        // Validate data quality for individual stocks
        ...
        // Validate data quality for stock arrays
        if (Array.isArray(result) && result.length > 0) {
          const validStocks = result.filter(stock => stock.price && parseFloat(stock.price) > 0);
          this.log(`${endpoint.name} returned ${validStocks.length}/${result.length} valid stocks`, 'success');
        }
      }
    }
